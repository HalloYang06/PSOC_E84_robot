...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--tee-log",
        default=None,
        help="If set, write the prompt + provider stdout/stderr to this file for CLI observability. "
             "Pass '-' to print prompt+output to stderr (default behavior when this flag is omitted: silent).",
    )
    parser.add_argument(
        "--echo",
        action="store_true",
        help="Echo the prompt header and provider output to stderr so the user sees what the AI received and replied.",
    )
...
def _open_tee(path: str | None):
    """Return a file handle for tee-ing logs, or None."""
    if not path:
        return None
    try:
        p = Path(path)
        ...
        print(f"[tee-log] failed to open {path}: {exc}", file=sys.stderr)
        return None


def _emit_observability(*, header: str, body: str, tee_handle, echo: bool) -> None:
    """Write to tee log file and/or stderr so the user can see progress."""
    line = f"\n--- {header} ---\n{body}\n"
...
def main() -> int:
    ...
    tee_handle = _open_tee(args.tee_log)
    obs = lambda h, b: _emit_observability(header=h, body=b, tee_handle=tee_handle, echo=args.echo)

    obs(
        f"[{args.provider}] PROMPT to {args.workstation_id or 'workstation'} (msg={args.message_id or '-'})",
        final_prompt,
    )
    ...
    try:
        ...
        obs(f"[{args.provider}] ERROR", str(exc))
        if tee_handle is not None:
            tee_handle.close()
        raise

    obs(f"[{args.provider}] RAW OUTPUT", output)

    final_reply = _normalize_final_reply(output)
    obs(f"[{args.provider}] FINAL REPLY", final_reply)
