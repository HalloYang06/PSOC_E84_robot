...
extern rt_uint32_t ifx_i2s_tx_underflow_count(void);
extern rt_uint32_t ifx_i2s_tx_zero_fill_count(void);
extern rt_uint32_t ifx_i2s_tx_frame_ready_count(void);

#define VOICE_PCM_BUFFER_SIZE        (320000U)
#define VOICE_TTS_PENDING_SLOT_SIZE   (8192U)
#define VOICE_TTS_PENDING_SLOT_COUNT  (12U)
#define VOICE_TTS_PENDING_BUFFER_SIZE (VOICE_TTS_PENDING_SLOT_SIZE * VOICE_TTS_PENDING_SLOT_COUNT)
#define VOICE_SERVER_TEXT_SLOT_SIZE    (384U)
#define VOICE_SERVER_TEXT_SLOT_COUNT   (4U)
...
#define VOICE_TTS_PREBUFFER_MIN_SLOTS 2U
#define VOICE_TTS_PREBUFFER_MAX_MS   180U
#define VOICE_TTS_THREAD_PRIORITY    19U
#define VOICE_TTS_THREAD_WAIT_MS      5U
...
#error "VOICE_TTS_M55_WRITE_BLOCK_SIZE must not exceed RT_AUDIO_REPLAY_MP_BLOCK_SIZE"
#endif
...
#endif
#ifndef VOICE_TTS_PLAYBACK_TO_M33
...
#define VOICE_QA_TEXT_THREAD_STACK     6144
...
typedef struct
{
    ...
    char server_text_pending[VOICE_SERVER_TEXT_SLOT_COUNT][VOICE_SERVER_TEXT_SLOT_SIZE];
    volatile rt_uint32_t server_text_pending_len[VOICE_SERVER_TEXT_SLOT_COUNT];
    volatile rt_uint32_t server_text_pending_read_index;
    ...
    rt_uint32_t tts_replay_queue_high_water;
    rt_uint32_t tts_decode_max_ms;
    rt_uint32_t tts_write_max_ms;
    ...
    rt_device_t xiaozhi_speaker_dev;
    rt_bool_t xiaozhi_speaker_ready;
...
static rt_err_t voice_service_publish_status(void)
{
    ...
    if (g_service.xiaozhi_tts_forward_chunks > 0U)
    {
        msg.payload.voice_status.xiaozhi_server_last_text_lens =
            ((g_service.tts_pending_count & 0x3ffU) |
             ((g_service.tts_pending_high_water & 0x3ffU) << 10U) |
             ((g_service.tts_replay_queue_high_water & 0x3ffU) << 20U));
        msg.payload.voice_status.xiaozhi_server_last_error_code =
            ((g_service.tts_decode_max_ms & 0xffffU) |
             ((g_service.tts_write_max_ms & 0xffffU) << 16U));
        msg.payload.voice_status.xiaozhi_server_last_reason_code =
            ((g_service.tts_sound_wait_timeout_count & 0xffffU) |
             ((g_service.tts_speaker_busy_count & 0xffffU) << 16U));
    }
...
void voice_service_dump_tts_diag(void)
{
    ...
    rt_uint32_t replay_high;
    rt_uint32_t decode_max_ms;
    rt_uint32_t write_max_ms;
    ...
    replay_high = g_service.tts_replay_queue_high_water;
    decode_max_ms = g_service.tts_decode_max_ms;
    write_max_ms = g_service.tts_write_max_ms;
    ...
    rt_kprintf("tts_diag sound wait_timeout=%lu busy=%lu replay_high=%lu decode_max=%lums write_max=%lums pending_len=%lu last_ret=%d phase=%lu consume=%d heap=%lu/%lu max=%lu\n",
               (unsigned long)wait_timeout,
               (unsigned long)speaker_busy,
               (unsigned long)replay_high,
               (unsigned long)decode_max_ms,
               (unsigned long)write_max_ms,
    ...
    rt_kprintf("tts_diag audio zero=%lu partial=%lu qfail=%lu mpfail=%lu i2s_under=%lu i2s_zero=%lu i2s_ready=%lu\n",
               (unsigned long)rt_audio_replay_zero_frame_count(),
               (unsigned long)rt_audio_replay_partial_under_run_count(),
               (unsigned long)rt_audio_replay_queue_push_fail_count(),
               (unsigned long)rt_audio_replay_mp_alloc_fail_count(),
...
static rt_bool_t voice_service_publish_tts_audio_to_m33(const uint8_t *audio_data,
                                                        uint32_t len)
{
    m33_m55_message_t msg;
    uint32_t offset = 0U;
    uint32_t chunk_index = 0U;
    rt_bool_t sent_any = RT_FALSE;

    if ((audio_data == RT_NULL) || (len == 0U))
    {
        return RT_FALSE;
    }
    ...
    while (offset < len)
    {
        ...
        rt_memcpy(msg.payload.audio_data.data, audio_data + offset, chunk_len);
        ...
        if (ret != RT_EOK)
        {
            ...
            rt_mutex_release(&g_service.lock);
            rt_kprintf("[voice_service] publish TTS audio to M33 failed ret=%d idx=%lu len=%lu\n",
                       ret,
...
static void voice_service_flush_tts_audio_to_m33(void)
{
    m33_m55_message_t msg;
...
static rt_err_t voice_service_open_m55_speaker(void)
{
    struct rt_audio_caps caps;
    rt_err_t ret;

    if (g_service.xiaozhi_speaker_ready && (g_service.xiaozhi_speaker_dev != RT_NULL))
    {
        return RT_EOK;
    }

    if (g_service.xiaozhi_speaker_dev == RT_NULL)
    {
        g_service.xiaozhi_speaker_dev = rt_device_find("sound0");
        if (g_service.xiaozhi_speaker_dev == RT_NULL)
        {
            g_service.xiaozhi_speaker_last_ret = -RT_ERROR;
            rt_kprintf("[voice_service] M55 sound0 not found for Xiaozhi speaker\n");
            return -RT_ERROR;
        }
    }
    ...
    {
        g_service.xiaozhi_speaker_last_ret = ret;
        rt_kprintf("[voice_service] configure M55 sound0 failed ret=%d\n", ret);
        return ret;
    }

    ret = rt_device_open(g_service.xiaozhi_speaker_dev, RT_DEVICE_OFLAG_WRONLY);
    if ((ret != RT_EOK) && (ret != -RT_EBUSY))
    {
        g_service.xiaozhi_speaker_last_ret = ret;
        rt_kprintf("[voice_service] open M55 sound0 failed ret=%d\n", ret);
        return ret;
    }
...
static rt_bool_t voice_service_wait_m55_speaker_ready_to_write(rt_uint32_t timeout_ms)
{
    rt_tick_t deadline;
    ...
    audio = (struct rt_audio_device *)g_service.xiaozhi_speaker_dev;
    if ((audio == RT_NULL) || (audio->replay == RT_NULL))
    {
        return RT_TRUE;
    }
    ...
    {
        rt_uint32_t qlen = (rt_uint32_t)rt_data_queue_len(&audio->replay->queue);
        if (qlen > g_service.tts_replay_queue_high_water)
        {
            g_service.tts_replay_queue_high_water = qlen;
        }
        if ((rt_int32_t)(deadline - rt_tick_get()) <= 0)
        {
            rt_kprintf("[voice_service] M55 sound0 queue busy timeout q=%u\n",
...
static rt_bool_t voice_service_stream_pcm_to_m55_speaker(const uint8_t *audio_data,
                                                        uint32_t len,
                                                        rt_bool_t validate_raw_packet)
{
    ...
    if ((len > 44U) && (rt_memcmp(audio_data, "RIFF", 4) == 0))
    {
        payload_offset = 44U;
    }
        ...
        !voice_service_audio_looks_like_pcm16(audio_data + payload_offset, len - payload_offset))
    {
        ...
        rt_mutex_release(&g_service.lock);
        rt_kprintf("[voice_service] M55 speaker rejected non-pcm len=%lu head=%02x %02x %02x %02x\n",
                   (unsigned long)len,
...
#if VOICE_TTS_PLAYBACK_TO_M33
    return voice_service_publish_tts_audio_to_m33(audio_data + payload_offset,
                                                 len - payload_offset);
#elif !VOICE_TTS_PLAYBACK_TO_M55
    if ((g_service.xiaozhi_rx_binary_count <= 3U) ||
        ((g_service.xiaozhi_rx_binary_count % 20U) == 0U))
    {
        rt_kprintf("[voice_service] TTS audio muted len=%lu; M55 sound0 blocks and M33 audio is disabled\n",
                   (unsigned long)(len - payload_offset));
    }
    return RT_TRUE;
#endif

    if (voice_service_open_m55_speaker() != RT_EOK)
    {
        g_service.service_diag_phase = 5201U;
        g_service.service_last_consume_ret = g_service.xiaozhi_speaker_last_ret;
        g_service.xiaozhi_server_last_error_code =
            ((5201U & 0xffffU) << 16U) | ((rt_uint32_t)(-g_service.xiaozhi_speaker_last_ret) & 0xffffU);
        g_service.xiaozhi_server_last_reason_code = len;
        return RT_FALSE;
    }

    if (g_service.xiaozhi_speaker_pending == RT_NULL)
    {
        g_service.xiaozhi_speaker_pending = (rt_uint8_t *)rt_malloc(VOICE_TTS_M55_WRITE_BLOCK_SIZE);
        if (g_service.xiaozhi_speaker_pending == RT_NULL)
        {
            g_service.xiaozhi_speaker_last_ret = -RT_ENOMEM;
            g_service.service_diag_phase = 5207U;
            g_service.service_last_consume_ret = -RT_ENOMEM;
            g_service.xiaozhi_server_last_error_code =
                ((5207U & 0xffffU) << 16U) | ((rt_uint32_t)RT_ENOMEM & 0xffffU);
            g_service.xiaozhi_server_last_reason_code = len;
            rt_kprintf("[voice_service] alloc M55 sound0 pending failed size=%lu\n",
                       (unsigned long)VOICE_TTS_M55_WRITE_BLOCK_SIZE);
            return RT_FALSE;
        }
        g_service.xiaozhi_speaker_pending_len = 0U;
    }

    while (offset < (len - payload_offset))
    {
        uint32_t space = VOICE_TTS_M55_WRITE_BLOCK_SIZE - g_service.xiaozhi_speaker_pending_len;
        uint32_t copy_len = (len - payload_offset) - offset;

        if (copy_len > space)
        {
            copy_len = space;
        }
        rt_memcpy(g_service.xiaozhi_speaker_pending + g_service.xiaozhi_speaker_pending_len,
                  audio_data + payload_offset + offset,
                  copy_len);
        g_service.xiaozhi_speaker_pending_len += copy_len;
        offset += copy_len;

        if (g_service.xiaozhi_speaker_pending_len >= VOICE_TTS_M55_WRITE_BLOCK_SIZE)
        {
            ...
            {
                rt_tick_t write_start = rt_tick_get();
                rt_uint32_t write_ms;

                written = rt_device_write(g_service.xiaozhi_speaker_dev,
                                          0,
                                          g_service.xiaozhi_speaker_pending,
                                          VOICE_TTS_M55_WRITE_BLOCK_SIZE);
                write_ms = (rt_uint32_t)((rt_tick_get() - write_start) * 1000U / RT_TICK_PER_SECOND);
                if (write_ms > g_service.tts_write_max_ms)
                {
                    g_service.tts_write_max_ms = write_ms;
                }
            }
            ...
            {
                g_service.xiaozhi_speaker_last_ret = -RT_ERROR;
                g_service.service_diag_phase = 5203U;
                g_service.service_last_consume_ret = -RT_ERROR;
                g_service.xiaozhi_server_last_error_code =
                    ((5203U & 0xffffU) << 16U) | ((rt_uint32_t)RT_ERROR & 0xffffU);
                g_service.xiaozhi_server_last_reason_code = len;
                rt_mutex_take(&g_service.lock, RT_WAITING_FOREVER);
                g_service.xiaozhi_tts_forward_fail_count++;
                rt_mutex_release(&g_service.lock);
                rt_kprintf("[voice_service] M55 sound0 write failed len=%lu\n",
                           (unsigned long)VOICE_TTS_M55_WRITE_BLOCK_SIZE);
                return RT_FALSE;
            }
            ...
            rt_mutex_release(&g_service.lock);
            g_service.service_diag_phase = 5204U;
            g_service.service_last_consume_ret = (rt_int32_t)written;
            if ((g_service.xiaozhi_tts_forward_chunks <= 3U) ||
                ((g_service.xiaozhi_tts_forward_chunks % 20U) == 0U))
            {
                rt_kprintf("[voice_service] tts->m55 sound0 chunk=%lu written=%lu len=%lu\n",
                           (unsigned long)g_service.xiaozhi_tts_forward_chunks,
...
static rt_bool_t voice_service_flush_m55_speaker(void)
{
    rt_size_t written;

    if ((g_service.xiaozhi_speaker_pending_len == 0U) ||
        (g_service.xiaozhi_speaker_dev == RT_NULL) ||
        (g_service.xiaozhi_speaker_pending == RT_NULL))
    {
        return RT_TRUE;
    }
    ...
    rt_memset(g_service.xiaozhi_speaker_pending + g_service.xiaozhi_speaker_pending_len,
              0,
              VOICE_TTS_M55_WRITE_BLOCK_SIZE - g_service.xiaozhi_speaker_pending_len);
    if (official_voice_speaker_take(voice_service_ms_to_ticks(VOICE_TTS_REPLAY_WAIT_MS)) != RT_EOK)
    {
        g_service.xiaozhi_speaker_last_ret = -RT_EBUSY;
    ...
    if (written == 0U)
    {
        g_service.service_diag_phase = 5205U;
        g_service.service_last_consume_ret = -RT_ERROR;
        return RT_FALSE;
    }
    ...
    g_service.xiaozhi_tts_forward_bytes += (rt_uint32_t)written;
    rt_mutex_release(&g_service.lock);
    g_service.service_diag_phase = 5206U;
    g_service.service_last_consume_ret = (rt_int32_t)written;
    return RT_TRUE;
}
...
static rt_bool_t voice_service_decode_opus_to_m55_speaker(const uint8_t *opus_data, uint32_t len)
{
    ...
    rt_size_t i;
    rt_tick_t decode_start = rt_tick_get();
    rt_uint32_t decode_ms;
    ...
    ret = xiaozhi_opus_decoder_decode(opus_data,
                                      ...
                                      &pcm_samples);
    decode_ms = (rt_uint32_t)((rt_tick_get() - decode_start) * 1000U / RT_TICK_PER_SECOND);
    if (decode_ms > g_service.tts_decode_max_ms)
    {
        g_service.tts_decode_max_ms = decode_ms;
    }
    if (ret != RT_EOK)
    {
        g_service.service_diag_phase = 5101U;
        g_service.service_last_consume_ret = ret;
        g_service.xiaozhi_audio_frame_len = len;
        g_service.xiaozhi_server_last_error_code =
            (((rt_uint32_t)((-ret) & 0xffff)) |
             (((rt_uint32_t)len & 0xffffU) << 16U));
        g_service.xiaozhi_server_last_reason_code =
            ((len > 0U ? (rt_uint32_t)opus_data[0] : 0U) |
    ...
    g_service.service_diag_phase = 5102U;
    g_service.service_last_consume_ret = (rt_int32_t)pcm_samples;
    g_service.xiaozhi_audio_frame_len = len;
    g_service.xiaozhi_server_last_error_code =
        ((rt_uint32_t)pcm_samples & 0xffffU) | (((rt_uint32_t)len & 0xffffU) << 16U);
    g_service.xiaozhi_server_last_reason_code =
        ((len > 0U ? (rt_uint32_t)opus_data[0] : 0U) |
         ((len > 1U ? (rt_uint32_t)opus_data[1] : 0U) << 8U) |
         ((len > 2U ? (rt_uint32_t)opus_data[2] : 0U) << 16U) |
    ...
    return voice_service_stream_pcm_to_m55_speaker((const uint8_t *)pcm16,
                                                   (uint32_t)(out_samples * sizeof(pcm16[0])),
                                                   RT_FALSE);
}

static rt_bool_t voice_service_decode_v3_opus_frames_to_m55_speaker(const uint8_t *payload, uint32_t len)
{
    uint32_t offset = 0U;
    uint32_t frames = 0U;
    ...
    while ((offset + XIAOZHI_BINARY_V3_HEADER_LEN) <= len)
    {
        ...
        if (voice_service_decode_opus_to_m55_speaker(frame + XIAOZHI_BINARY_V3_HEADER_LEN, frame_len))
        {
...
static void voice_service_enqueue_server_text(const uint8_t *payload, rt_size_t payload_len)
{
    rt_size_t copy_len;
    rt_uint32_t slot;

    if ((payload == RT_NULL) || (payload_len == 0U))
    {
        return;
    }
    ...
    if (g_service.server_text_pending_count >= VOICE_SERVER_TEXT_SLOT_COUNT)
    {
        ...
        rt_mutex_release(&g_service.lock);
        rt_kprintf("[voice_service] server text queue full drop len=%lu\n",
                   (unsigned long)payload_len);
        return;
    }

    slot = g_service.server_text_pending_write_index;
    rt_memcpy(g_service.server_text_pending[slot], payload, copy_len);
    g_service.server_text_pending[slot][copy_len] = '\0';
    g_service.server_text_pending_len[slot] = (rt_uint32_t)copy_len;
    g_service.server_text_pending_write_index =
        (slot + 1U) % VOICE_SERVER_TEXT_SLOT_COUNT;
    g_service.server_text_pending_count++;
    rt_mutex_release(&g_service.lock);
}

static rt_bool_t voice_service_process_pending_server_text(void)
{
    char text[VOICE_SERVER_TEXT_SLOT_SIZE];
    rt_uint32_t slot;
    rt_uint32_t len;

    rt_mutex_take(&g_service.lock, RT_WAITING_FOREVER);
    if (g_service.server_text_pending_count == 0U)
    {
        rt_mutex_release(&g_service.lock);
        return RT_FALSE;
    }

    slot = g_service.server_text_pending_read_index;
    len = g_service.server_text_pending_len[slot];
    if (len >= VOICE_SERVER_TEXT_SLOT_SIZE)
    {
        len = VOICE_SERVER_TEXT_SLOT_SIZE - 1U;
    }
    rt_memcpy(text, g_service.server_text_pending[slot], len);
    text[len] = '\0';
    g_service.server_text_pending[slot][0] = '\0';
    g_service.server_text_pending_len[slot] = 0U;
    g_service.server_text_pending_read_index =
        (slot + 1U) % VOICE_SERVER_TEXT_SLOT_COUNT;
    if (g_service.server_text_pending_count > 0U)
    {
        g_service.server_text_pending_count--;
...
static rt_bool_t voice_service_process_pending_tts(void)
{
    ...
    {
        g_service.xiaozhi_audio_frame_len = len;
        g_service.service_diag_phase = 5001U;
#if !VOICE_TTS_PLAYBACK_TO_M33 && !VOICE_TTS_PLAYBACK_TO_M55
        if ((g_service.xiaozhi_rx_binary_count <= 3U) ||
            ((g_service.xiaozhi_rx_binary_count % 20U) == 0U))
        {
            rt_kprintf("[voice_service] TTS binary muted before decode len=%lu\n",
                       (unsigned long)len);
        }
        streamed = RT_TRUE;
#else
        if (XIAOZHI_AUDIO_FORMAT_IS_PCM)
        {
            ...
            {
                streamed = voice_service_stream_pcm_to_m55_speaker(pcm_payload,
                                                           (uint32_t)pcm_len,
                                                           RT_FALSE);
            }
            else
            ...
            voice_service_payload_has_v3_audio_header(payload, len))
        {
            ...
            {
                rt_kprintf("[voice_service] v3 payload pcm16 to speaker len=%lu\n",
                           (unsigned long)pcm_len);
                streamed = voice_service_stream_pcm_to_m55_speaker(pcm_payload,
                                                           (uint32_t)pcm_len,
                                                           RT_TRUE);
            ...
            if (!streamed)
            {
                streamed = voice_service_decode_v3_opus_frames_to_m55_speaker(payload, len);
            }
        }
        if (!streamed && !XIAOZHI_AUDIO_FORMAT_IS_PCM)
        {
            streamed = voice_service_decode_opus_to_m55_speaker(payload, len);
        }
        if (!streamed && voice_service_audio_looks_like_pcm16(payload, len))
        {
            streamed = voice_service_stream_pcm_to_m55_speaker(payload, len, RT_TRUE);
        }
#endif
        if (!streamed)
        {
            g_service.service_diag_phase = (g_service.service_diag_phase < 5100U) ? 5009U : g_service.service_diag_phase;
            g_service.xiaozhi_server_last_error_code =
                (((rt_uint32_t)g_service.service_diag_phase & 0xffffU) << 16U) |
                ((rt_uint32_t)(-g_service.service_last_consume_ret) & 0xffffU);
            g_service.xiaozhi_server_last_reason_code = len;
            rt_mutex_take(&g_service.lock, RT_WAITING_FOREVER);
            g_service.xiaozhi_tts_forward_fail_count++;
            rt_mutex_release(&g_service.lock);
...
rt_err_t voice_service_qa_xiaozhi_text_turn(const char *text)
{
    ...
    if (ret == RT_EOK)
    {
        ...
        g_service.xiaozhi_listening_active = RT_TRUE;
        g_service.xiaozhi_listening_source = XIAOZHI_WAKE_SOURCE_MANUAL;
        g_service.xiaozhi_listening_bytes = 0U;
        g_service.xiaozhi_listening_chunks = 0U;
        g_service.xiaozhi_listening_start_tick = rt_tick_get();
        rt_memset(g_service.xiaozhi_listening_session_id, 0, sizeof(g_service.xiaozhi_listening_session_id));
        rt_strncpy(g_service.xiaozhi_listening_session_id,
                   session_id,
    ...
    rt_snprintf(json, sizeof(json),
                ...
                "\"mode\":\"manual\",\"text\":\"%s\",\"source\":\"m55_qa_text\","
    ...
    if (ret == RT_EOK)
    {
        ...
        rt_kprintf("[voice_service] Xiaozhi QA text turn sent session=%s start=%d text=%s\n",
                   session_id,
                   start_sent ? 1 : 0,
                   text);
    }
...
static void voice_service_qa_text_thread_entry(void *parameter)
{
    voice_qa_text_ctx_t *ctx = (voice_qa_text_ctx_t *)parameter;

    if (ctx != RT_NULL)
    {
        rt_err_t ret = voice_service_qa_xiaozhi_text_turn(ctx->text);

        rt_kprintf("[voice_service] async qa_text ret=%d len=%lu\n",
                   ret,
...
static rt_err_t voice_service_qa_xiaozhi_text_turn_async(const char *text)
{
    voice_qa_text_ctx_t *ctx;
    rt_thread_t thread;

    ctx = (voice_qa_text_ctx_t *)rt_calloc(1, sizeof(*ctx));
    if (ctx == RT_NULL)
    {
        return -RT_ENOMEM;
    }
    ...
    thread = rt_thread_create("xz_qa_txt",
                              voice_service_qa_text_thread_entry,
                              ctx,
                              VOICE_QA_TEXT_THREAD_STACK,
                              22,
                              10);
    if (thread == RT_NULL)
    {
...
static void voice_service_handle_server_text(const char *message)
{
    ...
    if (rt_strcmp(type, "tts") == 0)
    {
        if (rt_strcmp(state, "start") == 0)
        {
            ...
            g_service.tts_pending_count = 0U;
            g_service.tts_pending_high_water = 0U;
            g_service.tts_replay_queue_high_water = 0U;
            g_service.tts_decode_max_ms = 0U;
            g_service.tts_write_max_ms = 0U;
            g_service.xiaozhi_speaker_pending_len = 0U;
            g_service.tts_prebuffering = RT_TRUE;
            g_service.tts_prebuffer_start_tick = rt_tick_get();
            rt_mutex_release(&g_service.lock);
        ...
        if (rt_strcmp(state, "stop") == 0)
        {
            ...
            rt_mutex_release(&g_service.lock);
#if VOICE_TTS_PLAYBACK_TO_M33
            voice_service_flush_tts_audio_to_m33();
#else
    ...
    /*
     * XiaoZhi product playback must come from the platform's binary audio
     * stream (Opus/PCM depending on the negotiated protocol). Text-only relay
     * messages are useful for LVGL/M33 visibility, but falling back to a local
     * TTS backend masks protocol regressions and can leave the user hearing the
     * wrong audio path.
     */
    rt_kprintf("[voice_service] text reply only; no XiaoZhi binary audio received yet\n");
    voice_service_clear_xiaozhi_thinking();
    rt_mutex_take(&g_service.lock, RT_WAITING_FOREVER);
    g_service.xiaozhi_tts_speaking = RT_FALSE;
    g_service.xiaozhi_listening_active = RT_FALSE;
...
static void on_websocket_message(websocket_message_type_t type, const uint8_t *payload, rt_size_t payload_len)
{
    ...
    if (type == WEBSOCKET_MESSAGE_BINARY)
    {
        ...
        if (payload[0] != '{')
        {
            ...
            voice_service_enqueue_server_text(payload, payload_len);
        }
}
...
static void voice_service_handle_control(const voice_control_msg_t *control)
{
    ...
    case VOICE_CTRL_M55_SPEAKER_TONE:
        g_service.xiaozhi_server_last_error_code = 0x544e0001U;
        g_service.xiaozhi_server_last_reason_code = control->cmd;
        ret = m55_speaker_tone_internal(1200U);
        g_service.xiaozhi_server_last_error_code =
            0x544e0000U | ((rt_uint32_t)(-ret) & 0xffffU);
        g_service.xiaozhi_server_last_reason_code = control->cmd;
        rt_kprintf("[voice_service] M55 speaker tone ret=%d\n", ret);
        break;
    default:
        g_service.xiaozhi_server_last_error_code = 0x554e0000U | (control->cmd & 0xffffU);
        g_service.xiaozhi_server_last_reason_code = control->cmd;
...
static void voice_service_handle_config(const voice_config_msg_t *config)
{
    ...
    case VOICE_CONFIG_XIAOZHI_QA_TEXT:
        ret = voice_service_qa_xiaozhi_text_turn_async(config->value);
        rt_kprintf("[voice_service] config xiaozhi qa_text async ret=%d len=%lu\n",
                   ret,
                   (unsigned long)rt_strlen(config->value));
        break;
...
static void voice_service_thread_entry(void *parameter)
{
    ...
    while (g_service.running)
    {
        ...
        voice_service_drain_ipc_messages();
        for (rt_uint32_t i = 0U; i < 2U; i++)
        {
            if (!voice_service_process_pending_server_text())
            {
                break;
            }
        }
...
rt_err_t voice_service_init(const char *baidu_api_key, const char *baidu_secret_key)
{
    ...
    rt_sem_init(&g_service.tts_sem, "vtts", 0, RT_IPC_FLAG_PRIO);
...
rt_err_t voice_service_start(void)
{
    ...
    g_service.tts_thread = rt_thread_create("voice_tts",
                                            voice_service_tts_thread_entry,
                                            RT_NULL,
                                            VOICE_TTS_THREAD_STACK,
                                            VOICE_TTS_THREAD_PRIORITY,
                                            10);
