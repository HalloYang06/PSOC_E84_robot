...
## 2026-07-11 - CYW55500 power-up must wait for M55 runtime readiness

Symptoms:
- M33 stopped at `autobaud stage5 power high` with Secure `CFSR=0x00010000`, `HFSR=0x40000000`, PC `0xEFFFFFFE`, and `g_runtime.loop_count=2`.
- The blue M33 heartbeat disappeared and the red fault LED remained visible.

Root cause:
- Minimal M33 startup initialized M33/M55 IPC after a one-second delay but powered the Bluetooth controller immediately.
- CYW55500 Bluetooth power-up collided with CM55/WiFi startup. UART IRQ installation and the second 100 ms delay were not the root cause.

Fix / reusable trick:
- Wait for the first M55 `VOICE_STATUS` snapshot before HCI autobaud and controller patch download.
- Keep HCI startup on the existing main startup stack. Moving it to a dynamically allocated 8 KB RT-Thread stack reproduced `CFSR=0x00010001`, `HFSR=0x40000000`, PC `0xEFFFFFFE`, and `loop_count=2`.
- After a 20-second M55 readiness timeout, skip Bluetooth and let the minimal heartbeat loop continue.
- Do not combine this with BLE command execution; keep BLE readonly until its transport is stable.

Status:
- Fixed and hardware-validated after reverting the asynchronous HCI experiment. Main-context startup reached patch download, `Bluetooth stack ready`, NUS advertising, and zero Secure/Non-secure Fault registers.

## 2026-07-11 - Reused sample BLE addresses can expose the wrong Windows GATT database

Symptoms:
- Windows found the board but showed the old Hello Sensor service `2f81c7b6-7447-46c2-a4c7-1ea55f2e2838` instead of NUS, while the firmware ELF contained the correct NUS UUIDs.

Root cause:
- Firmware used the hard-coded sample address `00:A0:50:11:44:77`; Windows reused a cached GATT database for that identity.

Fix / reusable trick:
- Derive a stable per-board random-static address from `Cy_SysLib_GetUniqueId()` and set the two MSBs of the first byte (`0xC0`).
- Use `BLE_ADDR_RANDOM`; do not share one sample address across boards.

Validation:
- The current board appeared as `C0:7A:D4:80:06:12`, exposed the NUS service/characteristics, and returned `OK:heartbeat`.

Status:
- Fixed.

## 2026-07-11 - A stale COM number is not proof that KitProg is connected

Symptoms:
- OpenOCD loads the correct SMIF bank configuration but reports `unable to find a matching CMSIS-DAP device`.
- Windows history still lists COM26, but opening it fails with `FileNotFoundError`.

Root cause:
- The KitProg composite device and its CMSIS-DAP/UART interfaces are phantom devices, not present USB devices.

Fix / reusable trick:
- Check `Get-PnpDevice -PresentOnly` for `USB\VID_04B4*` before debugging OpenOCD arguments or changing firmware.
- Require both a present CMSIS-DAP interface and an openable live UART before flashing/QA.
