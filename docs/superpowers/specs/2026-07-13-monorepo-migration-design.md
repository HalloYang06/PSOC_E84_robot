...
| `platform/shared/` | 同一 `ai-` 分支 | `f6c2c026` | Web/API 共同使用的类型和平台基础代码 |
| `platform/deploy/` | 同一 `ai-` 分支 | `f6c2c026` | 康复平台所需的容器与部署配置 |
...
每个正式来源都以审计确认的原始提交 SHA 直接接入目标仓库，不改写任何来源提交。迁移时先获取来源 ref 并确认实时 tip，再执行 `git merge -s ours --allow-unrelated-histories --no-commit <source-sha>`，使原始来源 tip 成为集成提交的第二父提交；随后用 `git read-tree --prefix=<target>/ -u <source-sha>[:source/subdir]` 把完整来源 tree 或选定子目录写入目标前缀，最后完成该 merge commit。这样最终 `main` 的当前 tree 按目标目录组织，同时来源 tip 及其全部原始祖先提交以完全相同的 SHA 成为 `main` 的祖先。
...
来源提交的 tree、父子关系和提交对象均不改变，因此原始提交哈希不会变化。`docs/migration/source-map.md` 和 `docs/migration/source-map.json` 记录来源仓库、来源 ref、准确的来源提交 SHA、目标目录和对应的集成 merge commit SHA。

迁移执行期间只使用本地 worktree 分支，不创建或推送远端 history/migration 分支；完成后目标远端最终只包含一个 `main`。所有组件的原始来源提交都是 `main` 的祖先。由于来源历史中的文件位于原仓库根路径，而集成后的当前 tree 位于新前缀，针对新路径执行普通路径日志会从集成提交开始；原始历史仍可通过 source map 中的 SHA、`git show <source-sha>`、`git log <source-sha>` 或集成提交的第二父历史完整导航。
...
- 各正式来源的历史均为 `main` 的祖先；
- 对每个来源执行 `git cat-file -e '<source-sha>^{commit}'` 和 `git merge-base --is-ancestor <source-sha> HEAD`；
- 抽查集成提交的第二父提交与 source map 中的准确来源 SHA 一致；
- 作者、时间和提交说明与来源历史一致；
- 来源映射无缺项。
