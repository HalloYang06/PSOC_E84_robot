...
| 消息左边色条 / 角色标签 | CLI / 灰蓝色 | watcher、Codex、Claude Code 或 Runner 的桥接消息 | 代表真实线程/本机执行器的状态回传 |
...
| 标签 | 含义 | 典型状态 |
|---|---|---|
| 派单 | 已写入协作消息池，等待目标线程处理 | `queued` / `pending` |
| 已接单 | adapter 或目标线程已经接住这条派单 | `acked` |
| 处理中 | 绑定线程正在推进，平台只等精简回执 | `in_progress` |
| 需人审 | 这条消息必须人工通过后才能进入目标线程 | `pending_review` |
| 已完成 | 目标线程已经返回最终结果 | `completed` / `done` |
| 异常 | 线程、Runner 或平台桥接报告失败 | `failed` / `rejected` |
