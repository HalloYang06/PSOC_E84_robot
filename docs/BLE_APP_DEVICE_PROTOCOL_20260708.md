...
# Hardware status notice (2026-07-11)

This document describes the intended App BLE/NUS contract. It is not evidence that the current M33 product image advertises the service. Hardware testing found a Secure HardFault when the current vendor BTSTACK HCI path was enabled, so the safe baseline keeps `M33_ENABLE_BT_HCI=0` until the library/RT-Thread port is corrected and the full acceptance gate passes.
