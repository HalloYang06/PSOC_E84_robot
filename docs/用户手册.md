...
- The board derives a stable random-static address from `Cy_SysLib_GetUniqueId()` to avoid Windows reusing another board's cached GATT database.
...
Readonly boundary:
- `move:*`, `mode:*`, and `stop` return `ERR:readonly` and are not queued.
- BLE callbacks and the BLE worker do not call motor enable, mode, position, velocity, torque, or M33/M55 IPC command APIs.
- Formal motion remains `JointTrajectory -> NanoPi -> M33 -> motor`.

Safe validation order:
1. Confirm the blue P16.5 heartbeat keeps blinking and the active-low red P16.7 LED is off.
2. Confirm `m55qa_status` reports `ipc_ready=1` and `tx_pending=0`.
3. Scan and connect to `OpenClaw-NUS`, subscribe to TX, then write `heartbeat`.
4. Write `stream:on` and require valid JSON frames with `sf=1` when feedback is missing or stale.
5. Write `stream:off`; no telemetry should arrive after one second.
6. Disconnect and confirm advertising resumes.

Do not send movement, motor enable, speed, position, or torque commands during this QA.
