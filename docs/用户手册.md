...
## M33 BLE hardware status correction (2026-07-11)

The current safe M33 baseline initializes the App BLE service and bounded command queue, but keeps `M33_ENABLE_BT_HCI=0`. The device must not be expected to advertise `OpenClaw-NUS` until the vendor BTSTACK/RT-Thread compatibility issue is fixed and revalidated on hardware.

Do not enable HCI on a wearable/demo build merely because compilation and flash verification pass. The acceptance gate is: normal cold boot, no red fault LED, BLE advertising and NUS RX/TX, plus continuously healthy CAN and M33/M55 IPC status. No motion command is allowed during that gate.
