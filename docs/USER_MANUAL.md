...
Use `care_summary` for home overview counters and status chips. It reports start readiness, active/finished/cancelled sessions, report review count, open AI drafts, queued offline evidence, blockers, `blocker_details` with user-facing title/description, `severity`, and `related_action_codes`, plus `primary_blocker` for the highest-priority blocker. It is a backend-composed summary of persisted records.
