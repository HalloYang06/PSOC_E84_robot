...
从上电开始完整走一遍 `电机/M33 -> NanoPi -> 仿真主机 -> MuJoCo hardware shadow`，优先按 [M33_NANOPI_MUJOCO_POWERON_TEST_GUIDE.md](M33_NANOPI_MUJOCO_POWERON_TEST_GUIDE.md) 执行。该文档明确区分当前已对上的 7 号外部电机 shadow 链路，以及尚未完成的 medical_arm 6DOF 正式 M33 映射。

真实产品一上电应自动启动状态服务，不应靠 SSH 手动启动。当前自启动模板见 [PRODUCT_AUTOSTART_GUIDE.md](PRODUCT_AUTOSTART_GUIDE.md)：NanoPi 默认自动启动只读 bridge，固定 `enable_target_tx=false`；仿真主机的 MuJoCo hardware shadow 是研发模式可选自启。

2026-06-03 最新实测：NanoPi `rehab-arm-nanopi-readonly.service` 已 `enabled/active`，仿真主机 `rehab-arm-sim-host-shadow.service` 已 `enabled/active`。从上电后的基础期望是：NanoPi 自动拉起 `can0`，自动发布 `/rehab_arm/motor_state`、`/rehab_arm/safety_state` 和当前 7 号外部电机对应的 `/joint_states forearm_rotation_joint`；仿真主机通过无线 ROS2 自动得到 `/sim/medical_arm/joint_trajectory` 和 `/sim/medical_arm/joint_states` 六关节 shadow。只读服务期间 `candump can0,320:7FF` 必须没有输出。
...
2026-06-03 已实测 7 号外部电机链路：ROS joint4 能经 M33 到 motor7，并通过 `0x334` 和 NanoPi `/joint_states forearm_rotation_joint` 上行到 MuJoCo hardware shadow。其他正式 6DOF 关节尚未全部完成 M33 对齐和标定。
...
默认左/右摄像头节点是 `/dev/video45` 和 `/dev/video47`，输出保存在 `~/rehab_arm_stereo_frames`。通过标准是平台返回 `ok=true`，并且 payload 的 `control_boundary` 为 `stereo_vision_context_only_not_motion_permission`。如果重启后 USB 摄像头没有绑定到 `uvcvideo`，可以临时加 `--ensure-uvc-module`，它只加载板上已有的 `/lib/modules/6.1.141.can-new/kernel/drivers/media/usb/uvc/uvcvideo.ko`；不要编译、替换或升级内核。该流程只提供视觉上下文，不发布 ROS 运动、不发 CAN、不改变 M33 状态。
...
只学习仿真时，优先按 [MEDICAL_ARM_MUJOCO_LEARNING_GUIDE.md](MEDICAL_ARM_MUJOCO_LEARNING_GUIDE.md) 走。那里只保留当前 6DOF MuJoCo 主线，不混入旧 5 关节 demo。
...
若现场确认方向和机械空间都安全，`-1.0 / 20s` 也已作为台架长一点的观察动作跑通：

```bash
python3 /home/pi/nanopi_can_master.py private active-report --iface can0 --motor 4 --enable-report --wait 0
python3 /home/pi/nanopi_can_master.py private speed --iface can0 --motor 4 --vel -1.0 --kd 1.0 --wait 0
sleep 20
python3 /home/pi/nanopi_can_master.py private stop --iface can0 --motor 4 --clear-fault --wait 0
python3 /home/pi/nanopi_can_master.py private active-report --iface can0 --motor 4 --wait 0
```
...
如果电源电流仍只有约 `0.1A`，不要继续只加 `limit_cur`。`0x7018 limit_cur` 是电流上限，不是电流命令；速度模式或 MIT 前馈力矩没有形成足够控制误差时，电机不会主动拉到该电流。

下一步应做“官方电流模式”小步阶跃，而不是继续速度模式：

- 写 `run_mode(0x7005)` 到当前型号对应的 current mode。
- 小步写 `iq_ref(0x7006)`，例如从 `0.2A` 或更低开始。
- 每步只保持 1~2 秒。
- 结束立即 stop/disable。
- 同时解码原始 active-report 里的实际电流/力矩字段。
...
torque_ff=-5.0 Nm
duration=1s
MIT frame=0x01524905#80007ED600003333
```

`-5.0Nm` 是高前馈远程台架脉冲，只允许短时验证。若需要更长保持或更大助力，优先转为 M33 内部受限助力控制，或使用官方 current mode，并加入超时、限流/限矩、限速、限位、急停和温度保护。

用户确认后，5号又做过同参数 5 秒保持：
...
这是高前馈台架保持测试，不允许无超时运行。正式控制必须迁入 M33 安全层。
...
- `4/5`：灵足 RS00，RobStride CSP，当前关节命令比例 `1.0`，已配置但暂不放行真实执行。
- `6/7`：灵足 EL05，RobStride CSP，当前关节命令比例 `1.0`，`7` 已台架验证过正式 M33 路径。
...
- `gear_ratio=1.0` 对 4/5/6/7 是当前正确的 RobStride CSP 关节命令比例；`drive_internal_reduction_ratio=10.0/9.0` 只是型号资料，不用于额外倍乘关节目标。
