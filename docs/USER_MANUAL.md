...
Use `care_timeline` for the recent history surface. It aggregates persisted training plans, training sessions, training reports, AI training drafts, and queued offline evidence items into timeline entries with source ids, status, timestamp, and compact details. It is read-only continuity evidence.

The phone `我的 / 训练活动` calendar must be driven by `care_timeline`, not by local demo values. The calendar highlights days when any persisted rehab activity exists, including accepted AI plans, normal training plans, training sessions, reports, AI drafts, or offline evidence. It should still count this week's real `training_session` records separately so a plan is not mistaken for completed exercise. If there are no timeline records, the expected user-facing state is `暂无本周康复活动` with a next-step button such as `去创建计划`; do not show hardcoded months, fake heatmap cells, or fake weekly counts.
...
Current 1.0.14 internal-test account setup path:
