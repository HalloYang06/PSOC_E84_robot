...
### 2026-06-27 - Stereo keyframe evidence made real, not synthetic overlay

- Completed: updated `scripts/nanopi_stereo_vla_upload_loop.sh` so the finite C++ VLA-V loop uploads both latest left and right JPG keyframes after stereo context upload. Defaults: `UPLOAD_KEYFRAME=1`, `KEYFRAME_CAMERA_IDS=stereo_left,stereo_right`, `KEYFRAME_WIDTH=640`, `KEYFRAME_HEIGHT=480`.
- Completed: platform API now stores camera-id scoped latest files, so `stereo_left` and `stereo_right` no longer overwrite each other. New browser URLs are `/api/rehab-arm/v1/devices/{device_id}/camera/keyframes/stereo_left/latest/file` and `/stereo_right/latest/file`.
- Completed: the rehab-arm VLA page now uses side-specific real keyframe URLs for `V 视觉证据`. It only draws a target box when that side has a browser-accessible image and a bbox from the latest C++ stereo payload. It no longer uses the left keyframe as a fake right-eye fallback.
- Validation: rehab-arm unit test `python -m unittest rehab_arm_ros2_ws.src.rehab_arm_psoc_bridge.test.test_stereo_camera_capture_upload` passed with 34 tests. Platform API test `python -m pytest apps/api/tests/test_rehab_arm_sync.py -q` passed with 37 tests. Platform Web build passed.
- Live NanoPi validation: copied the script to `/home/pi/nanopi_stereo_vla_upload_loop.sh` and ran `COUNT=2 INTERVAL_SECONDS=1 START_SEQUENCE=450 VISION_IMPL=cpp UPLOAD_KEYFRAME=1 ...`. The run uploaded stereo context plus `stereo_left` and `stereo_right` keyframes. Cloud checks showed left image `58628` bytes, right image `65738` bytes, and `same=False`.
- Live target: latest payload detected `bottle` with left bbox `[286,364,45,114]`, right bbox `[287,350,46,128]`, left center `[308.5,421.0]`, right center `[310.0,414.0]`, and `horizontal_disparity_px=-1.5`.
