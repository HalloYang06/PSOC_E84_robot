...
患者、设备、App、平台、NanoPi、M33、M55 和 VLA 共同遵守的 profile 协议见：[PATIENT_DEVICE_PROFILE_PROTOCOL_V1.md](PATIENT_DEVICE_PROFILE_PROTOCOL_V1.md)。服务器机械臂总控台和 App 用户端的功能协议见：[COMMAND_CENTER_APP_PROTOCOL_V1.md](COMMAND_CENTER_APP_PROTOCOL_V1.md)。后续新增 App、平台或模型字段时，优先更新共享协议，不要在各端各自定义一套。
...
| `M55_MODEL_RESULT_PROTOCOL_V1.md` | M55 小模型编号、置信度、语音文本、模型摘要和 `/rehab_arm/model_state` |
| `M33_M55_IPC_BLE_FOUNDATION.md` | GitHub 工程对应关系、M33/M55 MTB-IPC queue、共享 PCM、M33 BLE 到 App 字段边界 |
| `M55_MODEL_DEPLOYMENT_GUIDE.md` | M55 WiFi 工程的小模型部署路径，复用 TFLite Micro、model_manager 和 m33_m55_comm |
| `VOICE_WAKE_TTS_PORTABILITY_GUIDE.md` | M55 语音采集、官方/开源 wake-word、服务器 API 中转和扬声器播报的可移植合同 |
| `REHAB_FUNCTIONAL_ROADMAP.md` | 康复训练 session、4 路 EMG 预留、语音、路径规划、MuJoCo dry-run 和云平台隔离的模块化路线 |
| `COMMAND_CENTER_APP_PROTOCOL_V1.md` | 服务器机械臂总控台、Three.js/URDF、摄像头、语音中转、VLA、接线检测、安全检测、急停按钮和 App 用户端边界 |
...
| `/rehab_arm/sensor_state` | `std_msgs/msg/String` JSON | 仿真节点、M33/C8T6 传感桥 | recorder、算法、总控台 | EMG、IMU、心率等传感和滤波摘要。 |
| `/rehab_arm/motor_state` | `std_msgs/msg/String` JSON | 仿真遥测桥或后续 M33 状态桥 | recorder、总控台、标注工具 | 电机/关节遥测，只是数据，不是命令。 |
| `/rehab_arm/model_state` | `std_msgs/msg/String` JSON | M33/M55 汇总桥、离线回放、服务器模型 | recorder、总控台、VLA 数据链路 | M55/服务器模型摘要和编号语义，只是建议，不是运动许可。 |
...
- 可选 topic：`/rehab_arm/motor_state`、`/rehab_arm/model_state`、`/rehab_arm/camera_keyframe`。
