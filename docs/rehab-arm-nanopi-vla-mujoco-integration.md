...
- Completed: onsite cloud config was corrected so XiaoZhi ASR has both `REHAB_ARM_XIAOZHI_ASR_BASE_URL` and `REHAB_ARM_XIAOZHI_ASR_API_KEY`; TTS-only configuration previously made real board audio end at `asr_not_configured`, so no recognized text, LLM reply, or meaningful human voice could be produced.
- Completed: XiaoZhi WebSocket disconnect handling no longer writes an empty `disconnect` session over the latest TTS/reply state after a completed turn. The cloud startup script now allows `UVICORN_WS_ARGS` so embedded-client websocket ping behavior can be tuned without code edits.
- Validation: `python -m pytest apps/api/tests/test_rehab_arm_sync.py -k "xiaozhi"` passed, including v1/raw-PCM default, explicit v3/Opus compatibility, Opus-to-ASR decode, stale ASR clearing, short-TTS rejection, and disconnect-after-TTS state preservation.
- Validation: cloud ASR/TTS runtime was checked with the deployed `.env`: `qwen3-asr-flash` was called successfully, and `qwen-tts` returned 110080 bytes of 16 kHz mono PCM for a Chinese test reply.
