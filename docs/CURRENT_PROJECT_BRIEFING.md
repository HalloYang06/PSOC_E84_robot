...
| 总控台/App 协议 | `docs/COMMAND_CENTER_APP_PROTOCOL_V1.md` | 服务器机械臂总控台、Three.js/URDF、摄像头、语音中转、VLA、接线检测、安全检测、急停按钮和 App 用户端边界 |
...
| Linux 仿真主机 | MuJoCo 6DOF、hardware shadow、无线 ROS2、dry-run、标定和数据采集 | 已通过无线 ROS2 收到 NanoPi `/joint_states`，并按装机 3/4/5/6 映射发布 `/sim/medical_arm/joint_states` |
| 服务器/VLA | 机械臂总控台、Three.js/URDF 只读渲染、摄像头、语音 API 中转、VLA、接线检测、安全检测、急停请求、数据/模型管理 | 本仓库只定义协议；正式实时安全不依赖服务器，急停请求必须等 M33 ack |
...
```text
3/4/5/6 装机电机状态 -> M33 0x330~0x333 -> NanoPi /joint_states
-> 无线 ROS2 -> Linux 仿真主机
-> MuJoCo 6DOF shadow 的 jian_hengxiang / jian_zongxiang / jian_xuanzhuan / zhou_zongxiang
```

重要边界：

- 7号 EL05 不在机械臂上，只能作为 bench-debug 外部台架电机，不再是 hardware shadow 默认来源。
- 当前是只读真机状态驱动 MuJoCo shadow，不是完整 6DOF 真机闭环运动控制。
- 1/2 号腕部电机还未接线，腕部两轴当前在 MuJoCo shadow 中保持 0 占位。
- 各真实关节还需要逐个补方向、零点、传动比、软/硬限位和患者 ROM。
...
| `jian_xuanzhuan_joint` | `motor_id=6` EL05 | 装机 6 号已进入 hardware shadow；7 号只作外部台架调试 |
...
- 3/4/5/6 装机电机只读状态已能进入 MuJoCo 6DOF hardware shadow；3 号常态 fresh，4/5/6 需开启电机主动遥测后 fresh。
...
- 不能说完整 6DOF 真机医疗机械臂已经闭环控制完成；当前已验证的是只读状态 shadow。
- 不能说真实 4 路 EMG 小模型已经训练完成。
- 不能说 VLA 已经能安全控制真机。
- 不能说 7号外部电机就是正式 6号机械臂关节，也不能让 7号默认驱动 hardware shadow。
...
7. 服务器按 `COMMAND_CENTER_APP_PROTOCOL_V1.md` 做总控台：Three.js/URDF 只读渲染、摄像头、语音中转、VLA、接线检测、安全检测和急停请求。
8. VLA 先只输出高层任务或 dry-run 轨迹候选，经过 MuJoCo 验证后再进入 M33 安全审核。
...
- 7号外部电机和 `nanopi_can_master.py` 属于 `bench-debug`，不得作为医疗臂主线默认映射。
