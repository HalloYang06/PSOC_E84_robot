...
服务器下发给 NanoPi 的 A 部分必须先进入高层队列入口，不允许直接发布 ROS 轨迹：

```http
POST /api/rehab-arm/v1/devices/{device_id}/actions/high-level
Content-Type: application/json
```
...
NanoPi 侧用 `check_server_action_command.py --payload action.json --queue-item --pretty` 做第一道入口质量门。通过后只生成 `nanopi_high_level_action_queue_item_v1`，下一跳是 `vla_candidate_gate -> mujoco_dry_run_review -> operator_review -> m33_safety_gate_preparation`。失败时不能进入 dry-run，更不能发 `0x320`。
