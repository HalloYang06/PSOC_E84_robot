...
1. 硬性安全规则：硬件、部署、真实运动、固件、ROS 写、Git 回退等先判必须人工确认或禁止。
2. 必需能力匹配：`required_capability` 必须命中 NPC 职责、skill 或知识库。
3. 工位和信任策略：同工位优先，跨工位默认找目标工位长或进入人工确认。
4. runner 和线程可用性：目标 NPC 必须有可用绑定线程，否则不能假装已 queued。
5. 当前负载：负载低者优先。
6. 用户偏好：用户指定的默认承接 NPC、信任关系、必须人工确认关系优先。
...
| `requires_review` | 是否需要人工确认 |
| `review_reason` | 为什么要确认或为什么可直接派发 |
| `route_risk` | 路由风险 |
...
- 同工位、低风险、已信任的 Need。
- 用户明确设置过这对 NPC 可直接派发。
- 用户在设备数据工作台当前调试窗口里主动发起的安全只读/采集类操作，例如扫描真实接口、开始采集、停止采集、用户终端输入。它们可以直接进入目标电脑队列；通用执行电脑命令、NPC/AI 代操作、写参数、真实运动、固件、部署、ROS 写/service/action 仍必须按风险策略进入人工确认。
...
| 修改优先级/风险 | 更新 Need 后重新计算确认策略 | 队列排序和确认文案同步变化 |
...
- `codex_desktop_ui`：默认可见通道。用户要求“桌面版能实时看到”时必须优先使用，完整过程在绑定电脑的 Codex Desktop 线程里发生，平台只同步已收到提醒、最终结果和阻塞原因。它不能用后台 `app-server` 结果冒充桌面可见；后台线程跑完但桌面界面没显示，只能算后台执行成功，不能算桌面可见成功。
- `codex_app_server`：显式后台通道。它可以用于 Linux/VS Code 插件/无界面或用户明确允许的后台执行，也可以在不抢前台窗口的情况下恢复绑定线程执行一次，但不保证 Codex Desktop 当前界面实时显示。它仍必须由绑定的真实电脑 runner 执行。云端 API 服务器默认不能代替目标电脑启动 app-server，否则会把 Windows/Linux 派单抢到错误机器上；只有运维显式设置 `AI_COLLAB_ENABLE_SERVER_CODEX_APP_SERVER_AUTOSTART=1` 时，API 服务器才允许本机自启动。

桌面可见派单的启动链必须保持在目标电脑上：NPC 工作台点击“启动”时，云端只创建目标电脑 runner 队列项；如果该 NPC 的通道是 `codex_desktop_ui`，runner 队列项必须是“在这台电脑上把指定平台消息送入绑定桌面线程”的专用动作，由目标电脑 runner 调用本机适配器完成投递。用户单次派单的默认语义是“桌面自动化后台引导”：目标电脑 runner 必须优先写入 Codex Desktop 自己会检测的单次 heartbeat/automation 请求，让绑定线程像桌面版内置自动化一样自己运行；平台不能使用剪贴板、快捷键或强制切窗来打断用户当前操作。单次 heartbeat/automation 必须带过期时间，桌面线程确认收到后立即暂停，超过过期时间仍未接收时由 runner 扫描暂停，避免一次派单变成永久后台心跳。用户想看过程时，可以主动打开对应 Codex Desktop 线程查看。只有用户显式选择“抢占式应急同步”或运维显式设置抢占策略时，才允许用前台投递；抢占失败不能冒充已引导成功。后台 `app-server` 只能作为显式后台执行通道，不能退化为普通 runner 已收到提醒后就声称已进入桌面线程，也不能由云端 API/Web 服务器代跑桌面投递。

这条自动化链路暂时冻结为四层状态，后续前端优化不能改变语义：

1. `queued`：平台已创建目标电脑队列项，只表示排队。
2. `computer_received`：目标电脑 runner 已领取，只表示执行电脑在线。
3. `desktop_requested`：目标电脑已写入桌面单次自动化请求，只表示后台引导已登记，不能冒充用户已经看见。
4. `desktop_visible`：绑定桌面线程确认接收，才允许显示“桌面已收到/可见”。

如果只到前三层，NPC 工作台必须显示“后台接收请求已登记/等待桌面确认”，不能显示“桌面已收到”。用户普通单次派单不得使用剪贴板、SendKeys、前台聚焦、强制切窗；这些只能作为显式应急动作，并且必须在 UI 上说清楚会打断当前操作。

桌面可见的验收口径必须分三层，前端和回执不能混说：

- 执行电脑收到：runner 队列项被目标电脑领取，只能说明这台电脑在线。
- 本机收件箱写入：目标电脑把派单材料保存到 runner 工作目录，只能作为兜底证据，不能等同于用户在 Codex Desktop 看到了。
- 桌面线程确认可见：绑定 Codex Desktop 线程的后台通道确认本轮已进入该线程，或桌面通道确认出现这条派单，才允许显示“桌面已收到/可见”。如果只到前两层，NPC 工作台必须显示“执行电脑已收到，桌面线程未确认可见，可重新同步”，不能显示成功勾或最终投递成功。
...
```text
左侧：不常驻；部门/工位筛选只在需要时进入抽屉，避免挤占总览
中间：公司状态一览图 / 办公室沙盘
右侧：Token 用量、任务/对话明细、选中部门/NPC/电脑/阻塞项的详情/动作/证据抽屉
底部：最近运行事件和回执
```
...
- Token 经营面板：复用现有 `usage_logs` / `/api/usage`，按项目和当天聚合 `input_tokens + output_tokens` 为今日消耗，`cached_tokens` 作为缓存/本地复用节省口径；没有真实用量记录时显示“统计中”，不能编造数字。
...
- 能力工坊打开某个 NPC 瓷砖时，`Skill 配置` tab 第一屏必须先展示“该 NPC 当前拥有/上岗包会携带的 Skill”，而不是先展示全项目 Skill 池。每个 Skill 必须用用户能直接理解的来源标签：`NPC 自己创建`、`用户手动创建`、`用户导入本地 Skill`、`用户导入 GitHub 路径` 或 `平台基础`；同时显示 GitHub 仓库相对路径和可打开的 GitHub 链接。添加 Skill 的动作可以放在同 tab 下方或抽屉里，支持用户创建、登记本地 Skill、从 GitHub repo/blob/raw 路径导入并添加到当前 NPC。
- 能力工坊的 `知识库配置` tab 同样必须用明确来源标签：`NPC 自己创建`、`用户手动创建`、`用户导入本地知识库`、`用户导入 GitHub 路径` 或 `工位继承`；每条知识库都要显示 GitHub 仓库相对路径和可打开的 GitHub 链接。上岗包只读取这些配置源并把路径和来源说明清楚，不能把 Skill/知识库藏成一组无来源标签。
