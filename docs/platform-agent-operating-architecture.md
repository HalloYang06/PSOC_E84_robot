...
- `codex_desktop_ui`：默认可见通道。用户要求“桌面版能实时看到”时必须优先使用，完整过程在绑定电脑的 Codex Desktop 线程里发生，平台只同步最小回执、最终结果和阻塞原因。它不能用后台 `app-server` 结果冒充桌面可见；后台线程跑完但桌面界面没显示，只能算后台执行成功，不能算桌面可见成功。
