...
桌面可见派单的启动链必须保持在目标电脑上：NPC 工作台点击“启动”时，云端只创建目标电脑 runner 队列项；如果该 NPC 的通道是 `codex_desktop_ui`，runner 队列项必须是“在这台电脑上把指定平台消息送入绑定桌面线程”的专用动作，由目标电脑 runner 调用本机适配器完成投递。不能退化为普通 runner 最小回执后就声称已进入桌面线程，也不能由云端 API/Web 服务器代跑桌面投递。
...
- 桌面线程确认可见：绑定 Codex Desktop 会话文件或桌面通道确认出现这条派单，才允许显示“桌面已收到/可见”。如果只到前两层，NPC 工作台必须显示“执行电脑已收到，桌面线程未确认可见，可重新同步”，不能显示成功勾或最终投递成功。
