...
桌面可见派单的启动链必须保持在目标电脑上：NPC 工作台点击“启动”时，云端只创建目标电脑 runner 队列项；如果该 NPC 的通道是 `codex_desktop_ui`，runner 队列项必须是“在这台电脑上把指定平台消息送入绑定桌面线程”的专用动作，由目标电脑 runner 调用本机适配器完成投递。真正的目标是接入 Codex 桌面自带 heartbeat/automation 同款机制：目标是同一个线程、不打断用户当前操作、且桌面版能看到。后台 `app-server` 只能作为临时后台执行通道，不能退化为普通 runner 最小回执后就声称已进入桌面线程，也不能由云端 API/Web 服务器代跑桌面投递。
