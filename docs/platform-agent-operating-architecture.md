...
- `codex_desktop_ui`：默认可见通道。用户要求“桌面版能实时看到”时必须优先使用，完整过程在绑定电脑的 Codex Desktop 线程里发生，平台只同步最小回执、最终结果和阻塞原因。实现上优先使用 Codex 后台线程通道恢复同一个绑定线程并启动本轮处理，不抢前台窗口、不改用户剪贴板；只有目标电脑显式允许旧前台兜底时，才可以使用窗口聚焦、粘贴和快捷键兜底。
...
桌面可见派单的启动链必须保持在目标电脑上：NPC 工作台点击“启动”时，云端只创建目标电脑 runner 队列项；如果该 NPC 的通道是 `codex_desktop_ui`，runner 队列项必须是“在这台电脑上把指定平台消息送入绑定桌面线程”的专用动作，由目标电脑 runner 调用本机适配器完成投递。适配器必须优先走绑定线程的后台恢复/启动本轮处理，行为类似 Codex 桌面自带 heartbeat 自动化：目标是同一个线程，但不打断用户当前操作。不能退化为普通 runner 最小回执后就声称已进入桌面线程，也不能由云端 API/Web 服务器代跑桌面投递。
