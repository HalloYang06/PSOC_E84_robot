...
桌面可见派单的启动链必须保持在目标电脑上：NPC 工作台点击“启动”时，云端只创建目标电脑 runner 队列项；如果该 NPC 的通道是 `codex_desktop_ui`，runner 队列项必须是“在这台电脑上把指定平台消息送入绑定桌面线程”的专用动作，由目标电脑 runner 调用本机适配器完成投递。用户单次派单的默认语义是“抢占式桌面引导”：目标电脑 runner 必须尝试把平台指令直接送进同一个绑定 Codex Desktop 线程，让正在处理中的桌面线程被用户新指令引导；如果抢占投递没有被确认，平台不能显示“已引导成功”，只能显示待收口/重试。Codex Desktop heartbeat/automation 单次投递只能作为可选的非抢占队列模式，不能冒充“正在处理时已被引导”。后台 `app-server` 只能作为显式后台执行通道，不能退化为普通 runner 最小回执后就声称已进入桌面线程，也不能由云端 API/Web 服务器代跑桌面投递。
