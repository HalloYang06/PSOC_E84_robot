...
桌面可见派单的启动链必须保持在目标电脑上：NPC 工作台点击“启动”时，云端只创建目标电脑 runner 队列项；如果该 NPC 的通道是 `codex_desktop_ui`，runner 队列项必须是“在这台电脑上把指定平台消息送入绑定桌面线程”的专用动作，由目标电脑 runner 调用本机适配器完成投递。当前默认实现必须优先写入 Codex Desktop 能识别的单次 heartbeat/automation 投递：目标是同一个绑定线程、不使用剪贴板、不发送快捷键、不抢用户当前窗口，桌面版按自己的自动化节奏处理后由 runner 同步最终回执。后台 `app-server` 只能作为临时后台执行通道，不能退化为普通 runner 最小回执后就声称已进入桌面线程，也不能由云端 API/Web 服务器代跑桌面投递。旧的 SendKeys 前台粘贴只能作为显式调试兜底，默认禁用。
