...
- 右侧功能按钮点击后打开对应操作页或抽屉，例如 CAN 调试、串口调试、USB 调试、ROS 只读、run board、标注台、证据链。
...
P1 重构范围包括：驾驶舱、主页面、公司层、数据工厂、AI 实验室、机器人现场、观测台、能力工坊。NPC 工作台不属于这次推倒重构范围，只按已批准的 Need/Task tab 和去重规则做受控调整。
...
用途：机器人现场采集数据的入库、切片、标注、QA 和数据集版本工作台。

数据工厂不是抽象数据源管理页。它的主对象是“采集片段”：机器人现场通过串口、CAN、USB、SPI-CAN、ROS 只读、波形等接口，在同一时间段采到的电机、传感器、状态量、日志和波形数据。机器人现场负责采集和回执，数据工厂负责把这些同时间段多通道数据变成可标注、可组合、可训练的数据资产。
...
左侧：采集片段索引和数据集候选
中间：已打开的采集片段/标注瓷砖
右侧：变量选择、标注 schema、NPC 辅助、证据和动作
底部：入库日志、标注回执和 QA 事件
```

采集片段模型：

- 左侧显示按时间段采集到的数据片段，例如“2026-05-18 16:20:00-16:22:00，电机 1-6 + IMU + 电流 + 温度 + CAN 帧”。
- 采集片段来自机器人现场的真实接口采集结果，不能手工伪造一条看似已采集的数据。
- 每个采集片段可以像 NPC 瓷砖一样点击 `+` 打开，中间可同时打开多个片段瓷砖做对比、标注和 QA。
- 每个片段瓷砖展示时间范围、来源电脑、接口、采样率、通道列表、缺失/异常点、原始证据和已标注状态。
- 用户可以自由选择一个、两个或多个数据类型/变量/通道组成标注视图，例如电机电流 + 编码器速度 + IMU，或 CAN 帧 + 温度 + 末端力。
- 标注不固定成单一表单；必须支持区间标注、点标注、事件标注、分类标签、连续值标签、异常标签和备注。
- 用户可以把多个片段和多个变量组合成数据集候选，再创建数据集版本。
- 每个片段或数据集候选都可以绑定协助 NPC。NPC 可以辅助解释波形、建议标签、发现异常、生成标注草案，但不能自动最终确认高风险标签或最终 QA。
- 数据工厂产生的数据集版本、标注结果、QA 报告都必须落成 Artifact，并能回到机器人现场和 AI 实验室引用。
...
- Git 回退
- 硬件写操作
- 直接控制机器人现场接口采集；采集入口在机器人现场，数据工厂消费采集结果

### 4.7 AI 实验室

用途：采集数据的图表分析、变量关系探索、训练/评估实验和模型候选管理。

AI 实验室和数据工厂使用同一批采集片段与数据集版本。区别是：数据工厂负责标注和数据集质量，AI 实验室负责把数据转成图表、选择变量关系、跑训练/评估、比较指标和管理发布门。

布局：

```text
左侧：采集片段、数据集版本、实验 run
中间：已打开的分析/图表/实验瓷砖
右侧：变量映射、训练配置、NPC 辅助、发布门和证据
底部：训练/评估日志和指标回执
```

核心功能：

- 同步显示机器人现场采集片段和数据工厂数据集版本
- 点击 `+` 打开多个分析瓷砖或实验瓷砖
- 把采集数据转成图表
...
机器人现场按时间段采集电机/传感器/总线/波形数据
  ...
  -> 数据工厂左栏出现采集片段，用户打开瓷砖并选择多个变量/通道做 AI 辅助标注
  -> 人类确认标注和 QA，组合多个片段生成数据集版本
  -> AI 实验室左栏同步采集片段和数据集版本
  -> 用户打开图表/实验瓷砖，自由选择横纵坐标变量，跑训练/评估
  -> AI 实验室产出指标、失败样本和模型候选
  -> 失败样本回到数据工厂补标，或回到机器人现场复采同类时间段
  -> 观测台保留完整证据链
```
...
3. 专业工作台统一演进为 `ProfessionalWorkbenchShell` 或等价共享壳，数据工厂、AI 实验室、机器人现场、观测台、能力工坊不能各自做一套互不相干的 UI。
4. 右侧工具按钮打开专门操作面；不要把 CAN、串口、USB、ROS、标注、run board、证据链全部堆在中间。
...
7. 驾驶舱只保留今日决策、阻塞、待审、离线影响和可验收 final。
8. 公司层从 `WorkbenchClient` 拆出来，成为真正的工位树、员工表、职责/信任/能力绑定界面。
