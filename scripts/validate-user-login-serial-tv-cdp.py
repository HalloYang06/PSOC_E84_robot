...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate login -> home TV -> serial TV panel through Edge CDP.")
...
def main() -> int:
    ...
    profile_dir = Path(tempfile.mkdtemp(prefix="ai-collab-serial-tv-flow-edge-"))
    ...
    try:
        ...
        wait_for(
            cdp,
            "(() => { const frame = document.querySelector('iframe'); const win = frame && frame.contentWindow; return !!(win && Array.isArray(win.__platformSerialTvHotspotSnapshot) && win.__platformSerialTvHotspotSnapshot.length); })()",
            timeout_seconds=45,
        )
        time.sleep(1.0)
        ...
        hotspot_info = cdp_eval(
            ...
            (() => {
              ...
              const snapshot = win && Array.isArray(win.__platformSerialTvHotspotSnapshot)
                ? win.__platformSerialTvHotspotSnapshot.slice(0, 4)
                : [];
              return {
                iframeUrl: frame ? frame.src : '',
                hotspot: snapshot[0] || null,
                serialTextVisible: document.body.innerText.includes('串口电视')
              };
            })()
            """,
        )
        shot_home = output_dir / f"user-login-07-home-serial-tv-hotspot-{stamp}.png"
        ...
        click_point = cdp_eval(
            ...
            (() => {
              const frame = document.querySelector('iframe');
              const win = frame && frame.contentWindow;
              const hotspot = win && Array.isArray(win.__platformSerialTvHotspotSnapshot)
                ? win.__platformSerialTvHotspotSnapshot[0]
                : null;
        ...
        if not isinstance(click_point, dict) or not click_point.get("ok"):
            raise RuntimeError(f"Could not resolve serial TV click point: {click_point}")
        ...
        wait_for(
            cdp,
            "location.href.includes('tab=serial-tv') || !!document.querySelector('select[name=\\\"protocol\\\"]')",
            timeout_seconds=25,
        )
        time.sleep(1.0)
        shot_schedule = output_dir / f"user-login-08-serial-tv-panel-{stamp}.png"
        ...
        panel_info = cdp_eval(
            ...
            (() => ({
              url: location.href,
              hasSerialPanel: location.href.includes('tab=serial-tv') || document.body.innerText.includes('串口电视'),
              hasUsbScanForm: !!document.querySelector('select[name="computer_node_id"]'),
              hasProtocolForm: !!document.querySelector('select[name="protocol"]'),
              hasSerialWriteForm: !!document.querySelector('textarea[name="payload"]'),
              hasWaveformPreview: !!document.querySelector('svg[aria-label="串口数值波形预览"]'),
        ...
        report_path = output_dir / f"user-login-serial-tv-report-{stamp}.json"
