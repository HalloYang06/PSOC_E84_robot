...
static int cmd_m33_joint_calib(int argc, char **argv)
{
    ...
    {
        rt_kprintf("JOINT_CALIB: joint=%u motor_id=%u proto=%u calibrated=%u direction_x1000=%d gear_x1000=%d\n",
                   (unsigned int)joint,
    ...
    rt_kprintf("JOINT_CALIB_NOTE: M33 does not own zero labeling; upper platform/profile owns calibration metadata\n");
