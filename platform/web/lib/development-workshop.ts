...
const DEFAULT_MAP_SCENE = "workspace-map";
const DEFAULT_MAP_LOCATION = "项目工作区内";
...
export const DEFAULT_DEVELOPMENT_WORKSHOP_STATIONS: DevelopmentWorkshopStation[] = [
  {
    ...
    icon: "案",
    station: "项目办公室 / 需求入口",
    mapScene: "workspace-map",
    mapLocation: "项目入口与需求收集区之间",
    detail: "用户先说目标，平台拆成项目计划、模块、验收标准、任务链和可交给 AI 的工作单。",
    modes: ["项目开发", "教学辅助", "三维仿真", "知识训练"],
    ...
    nextActions: ["接需求入口", "生成需求草案", "把计划拆进任务链"],
  ...
  },
  {
    ...
    npcRoleTemplates: ["环境搭建员", "执行电脑能力巡检员", "依赖安装员"],
    assignmentKeywords: ["环境", "执行电脑", "依赖", "电脑", "environment-builder"],
    nextActions: ["接电脑管理器", "给执行电脑增加能力标签", "把环境状态写进项目配置"],
  ...
  },
  {
    ...
    station: "设备调试台 / 通讯控制台",
    mapScene: "toolbench",
    mapLocation: "设备调试工作台，也可从电脑接入管理进入",
    detail: "统一串口收发、USB 扫描、CAN 调试、日志采集和数字波形显示；协议先走 AICSV/1，后续兼容更多仪器。",
    modes: ["串口", "USB", "CAN", "日志", "波形"],
    backendAnchor: "/api/collaboration/projects/{project_id}/runner-commands",
    runnerCapabilities: ["serial-scan", "serial-open", "serial-write", "can-scan", "can-frame-read", "waveform-parse"],
    aiResponsibilities: ["解释日志", "生成只读检查命令", "判断波形异常", "沉淀实验记录"],
    npcRoleTemplates: ["设备调试员", "波形分析员", "日志解释员"],
    assignmentKeywords: ["串口", "波形", "USB", "CAN", "日志", "debug-console"],
    nextActions: ["接 Runner 串口/CAN 白名单", "把波形帧写入实验记录", "把写入类动作纳入人工审批"],
    approvalPolicy: "串口写入、CAN 注入、参数改写默认属于高风险动作，需要审批或现场确认。",
    riskLevel: "高",
    assignedNpcIds: [],
    knowledgeBase: {
  ...
  },
  {
    ...
    label: "AI 教练站",
    icon: "师",
    station: "NPC 管理 / 协作调度区",
    mapScene: "toolbench",
    mapLocation: "主页面 NPC 管理区与线程绑定区",
  ...
  },
  {
    ...
    icon: "仿",
    station: "仿真实验区 / 数字孪生台",
    mapScene: "simulation-lab",
    mapLocation: "仿真台与真机前检查区之间",
