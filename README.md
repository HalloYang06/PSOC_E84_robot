# Medical-Rehabilitation-Manipulator AI Branch

这个 `ai` 分支当前承载项目里的 VLA 任务理解子系统原型。当前版本不是直接驱动机械臂控制，而是负责把自然语言任务和场景状态解析成结构化的 `ResolvedTask`，供 ROS / MoveIt / 控制器继续规划、控制和执行。

## 当前边界

- VLA 负责任务理解、grounding、阶段判断、不确定性确认。
- ROS / MoveIt / 控制器负责轨迹规划、执行、安全约束和闭环控制。
