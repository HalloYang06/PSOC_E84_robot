            ...
            "release_gate": {
                ...
                "checks": [
                    ...
                    },
                    {
                        ...
                        "status": "pass",
                        "description": "Debug APK 1.0.17 loads public-config/catalog/workflow, uses Bearer token login, keeps profile phone binding synced after refresh, keeps Agent/device/profile tabs aligned with the current /me profile contract, fixes the home bottom community tab dead tap, overlays backend workflow/readiness, renders backend care timeline/daily care plan, binds the training library to backend plans after AI draft acceptance, adds one-tap Agent training advice from recent records, can execute safe workflow actions through /me/workflow/actions, includes Android SPP support, uses an http WebView scheme for the current http API, and includes local icon fallbacks so Material Symbols names are not shown when Google fonts fail.",
                    },
                    {
                    ...
                    },
                    {
                        ...
                        "description": "APK 1.0.17 includes a Capacitor/Android Bluetooth Classic SPP bridge plus a Stitch-designed Bluetooth debug page for listing paired devices, binding one as a backend trusted device, connecting SPP, sending backend-approved legacy frames, and uploading inbound ACK/sensor evidence. The device page also has an internal-test visible connection path when no native bridge is detected or permission/scan fails. User release still needs physical validation with current M33 firmware.",
                    },
                ],
