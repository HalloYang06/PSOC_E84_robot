...
type ThreeDDeveloperShellPageProps = {
  searchParams?: {
    projectId?: string;
    mode?: string;
  };
...
export default async function ThreeDDeveloperShellPage({ searchParams }: ThreeDDeveloperShellPageProps) {
  ...
  const { returnTo, modeBoardPath, currentProjectModePath, liveProjectPath, shellPath } =
    buildProjectFutureModeShellNavigation("3d-dev", selectedProjectId || undefined);
    ...
    <main className={styles.page}>
      <section className={styles.header}>
        <div className={styles.headerCopy}>
          <span className={styles.kicker}>3D 开发者占位壳</span>
          <h1>把 3D 开发者模式的分流后下一页也钉出来。</h1>
          <p>
            这里是 <code>3D 开发者模式入口</code> 在 <code>{modeBoardPath}</code> 之后的第一张真实占位壳页面。
            它还不是可用的 3D 开发环境，只是把“分流后会去哪里”推进成真实路由，而不再停留在模式板上的一张说明卡。
          </p>
          <p>
            当前 live 路径仍然是 `2D 开发者模式入口`，也就是当前项目页入口壳里的农场地图、背包抽屉和 NPC 协作开发面。
            这个占位壳只负责把未来 3D 开发模式的下游位置钉住，不替换现有农场底座。
          </p>

          <div className={styles.routePanel}>
            <p className={styles.routeEyebrow}>当前占位下游路径</p>
            <div className={styles.routeChain} aria-label="3d developer placeholder shell route">
            ...
            <p className={styles.routeNote}>
              下一步真正有价值的工作不再是补规则文案，而是把 3D 世界壳、协作桥接层和项目内执行面逐步接进这个占位壳。
            </p>
          </div>
        </div>

        <div className={styles.userCard}>
          <strong>{selectedProject ? selectedProject.name : "3D 开发分支占位"}</strong>
          <span>{selectedProject ? "当前正在查看这个项目的 3D 开发者下游占位壳" : "当前只固定 3D 开发模式的下游占位页"}</span>
      ...
      <section className={styles.panel}>
        <div className={styles.block}>
          <div className={styles.blockHead}>
            <h2>这个占位壳现在承担什么</h2>
            <p>它只做两件事：一是把 3D 开发者模式的下游入口位置钉成真实路由；二是明确告诉后续实现应该把什么接到这里。</p>
          </div>
          ...
          <div className={styles.itemList}>
            <article className={styles.projectCard}>
              <div>
                ...
                <p>认证、项目选择、模式分流板和 3D 开发者下游占位壳已经连成了一条真实路由链。</p>
              </div>
            ...
            <article className={styles.projectCard}>
              <div>
                ...
                <p>3D 世界壳、交互层、协作桥接层还没接进来，所以这个页面仍然不是可用的 3D 开发模式。</p>
              </div>
              <div className={styles.metaCol}>
                <span>3D 世界</span>
                <small>未开放</small>
