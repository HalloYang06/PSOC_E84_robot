...
def iter_candump_process(channel: str, protocol: str = PROTOCOL_LEGACY) -> Iterator[CanFrame]:
    ...
    cmd = ["candump", "-L", f"{channel},{','.join(filters)}"]
    ...
    with raw_path.open("w", newline="", encoding="utf-8") as raw_file, \
        ...
        try:
            ...
            while True:
                ...
                recording_event.set()
                print(f"[trial {trial.trial_index}] label={trial.label} recording... press Enter to stop")
                input()
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--source",
        choices=("python-can", "candump", "stdin", "file", "serial"),
        default="python-can",
        help="live python-can, live candump, stdin/file candump log, or M33 serial text",
    )
    parser.add_argument(
        "--protocol",
...
def _sleep_if_live(args: argparse.Namespace, seconds: float) -> None:
    if seconds <= 0:
        return
    if args.source in ("python-can", "candump"):
        time.sleep(seconds)
